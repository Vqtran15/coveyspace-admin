...
import { Smiley, Shield } from '@phosphor-icons/react'
import { broadcastAdminsPushAction, loadAdminsBroadcastHistoryAction } from '@/actions/admin'
...
function getMessage(entry) {
  return entry.metadata?.body ?? entry.target_label?.replace(/^\[Admins\] /, '') ?? ''
}

export default function BroadcastAdminsClient({ initialHistory }) {
  ...
  function handleSend() {
    ...
    startTransition(async () => {
      setError(null)
      const r = await broadcastAdminsPushAction({ title: title.trim() || undefined, body: draft.trim() })
      if (r.error) { setError(r.error); return }
      setTitle('')
      setDraft('')
      const result = await loadAdminsBroadcastHistoryAction()
  ...
  return (
    <div className="h-full flex flex-col bg-lagoon-50">
      <div className="bg-white border-b border-lagoon-100 px-6 py-4 shrink-0">
        ...
        <p className="text-xs text-stone-400 mt-0.5">Send a push notification exclusively to users with the admin role</p>
      </div>
      ...
      <main className="flex-1 overflow-auto p-6">
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          {/* History */}
          <div>
            <h3 className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-3">Recent Admin Broadcasts</h3>
            {history.length === 0 ? (
              <div className="bg-white rounded-2xl border border-stone-100 py-10 text-center text-stone-400">
                <p className="text-sm">No admin broadcasts sent yet</p>
              </div>
            ) : (
                ...
                {history.map(entry => (
                  <div key={entry.id} className="bg-white rounded-2xl border border-lagoon-100 px-5 py-4">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      {entry.metadata?.admin_count != null && (
                        <>
                          <span className="text-stone-200">·</span>
                          <span className="text-xs text-stone-400">{entry.metadata.admin_count} admins targeted</span>
                        </>
                      )}
                    </div>
                  </div>
